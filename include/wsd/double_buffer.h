...
private:
    ...
    RefCountedData* AcquireForReading()
    {
        ...
        while (true) {
            ...
            if (idx == m_read_idx.load(std::memory_order_acquire)) break;
            ReleaseReading(idx);
        }
        return &m_data[idx];
    }
...
public:
    DoubleBufferReader(DoubleBuffer<T>* db_p) : m_data(db_p->AcquireForReading())
    {
    }
...
public:
    DoubleBufferWriter(DoubleBuffer<T>* db_p) : m_db_p(db_p), m_data_p(m_db_p->AcquireForWriting())
    {
    }

    ~DoubleBufferWriter()
    {
        if (m_db_p) m_db_p->ReleaseWriting();
    }
