...
template <typename T>
struct is_future_type : std::false_type {
};
template <typename T>
struct is_future_type<Future<T>> : std::true_type {
};
...
template <typename T>
class PromptFutureObject : public FutureObjectInterface<T>, public std::enable_shared_from_this<PromptFutureObject<T>> {
public:
    ...
    PromptFutureObject(const std::exception_ptr& e) : m_exception_ptr(e)
    {
    }
    ...
    {
        if (m_exception_ptr) return false;
        return true;
    }
...
struct FutureObjectBase {
    FutureObjectBase() : m_is_done(false)
    {
    }
...
private:
    ...
    void doPendingCallbacks()
    {
        ...
        for (typename std::list<CallbackType>::const_iterator it = callbacks.begin(), end = callbacks.end(); it != end;
             ++it) {
            try {
...
private:
    ...
    void doPendingCallbacks()
    {
        ...
        for (std::list<CallbackType>::const_iterator it = callbacks.begin(), end = callbacks.end(); it != end; ++it) {
            (*it)(this->shared_from_this());
        }
    }
...
public:
    SequentialCallback(const Callback<R(const Future<T>&)>& callback, const Promise<value_type>& promise)
        : m_callback(callback), m_promise(promise)
    {
    }
    ...
    template <typename U>
    typename std::enable_if<!std::is_void<U>::value && !is_future_type<U>::value>::type run(const FuturePtr& future)
    {
        try {
...
class ForwardValue {
public:
    ForwardValue(const Promise<R>& promise) : m_promise(promise)
    {
    }
...
public:
    ...
    {
        if (!m_future) throw FutureUninitialized();
        return m_future->get();
    }
...
protected:
    ...
    FutureBase(const FuturePtr& future) : m_future(future)
    {
    }
...
public:
    ...
    Future(const std::exception_ptr& e) : detail::FutureBase<T>(e)
    {
    }
    ...
    Future<typename detail::resolved_type<R>::type> then(const Callback<R(const Future&)>& callback) const
    {
        ...
        if (!this->m_future) throw FutureUninitialized();
...
private:
    Future(const typename detail::FutureBase<T>::FuturePtr& future) : detail::FutureBase<T>(future)
    {
    }

    template <typename R, typename U>
    friend class detail::SequentialCallback;
    template <typename R>
    friend class detail::ForwardValue;
...
public:
    ...
    Future(const std::exception_ptr& e) : detail::FutureBase<void>(e)
    {
    }
...
private:
    Future(const detail::FutureBase<void>::FuturePtr& future) : detail::FutureBase<void>(future)
    {
    }
...
template <typename T>
typename std::enable_if<!std::is_void<T>::value, Future<T>>::type makeFuture(
        typename detail::FutureTraits<T>::rvalue_source_type t)
{
    return Future<T>(t);
}
...
class Promise {
public:
    Promise() : m_future(std::shared_ptr<detail::FutureObject<T>>(new detail::FutureObject<T>()))
    {
    }
...
public:
     ...
     */
    Promise() : m_future(std::shared_ptr<detail::FutureObject<void>>(new detail::FutureObject<void>()))
    {
    }
