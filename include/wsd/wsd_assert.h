...
{
    std::cerr << "at " << file << ':' << line << ": in " << function << "(): Assertion `" << expr << "' failed"
              << std::endl;
    std::abort();
}
...
{
    std::cerr << "at " << file << ':' << line << ": in " << function << "(): should not reach here" << std::endl;
    std::abort();
}
...
#define WSD_ASSERT(expr) ((expr) ? ((void) 0) : ::wsd::detail::assertion_failed(#expr, __func__, __FILE__, __LINE__))
...
#define STATIC_ASSERT(exp, msg)                                    \
    {                                                              \
        ::wsd::detail::CompileTimeError<((exp) != 0)> ERROR_##msg; \
        (void) ERROR_##msg;                                        \
    }
