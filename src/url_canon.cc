...
void backUpToPreviousSlash(int path_begin_in_output, std::string *output)
{
    ...
    if (i == path_begin_in_output) return;
...
void canonicalizeScheme(const char *input, int len, Component *scheme, std::string *output)
{
    ...
    if (scheme->isValid()) {
        for (int i = scheme->begin; i < scheme->end(); i++) *output += std::tolower(input[i]);
        *output += ':';
    }
}

void canonicalizeUserInfo(const char *input, int len, Component *username, Component *password, std::string *output)
{
...
void canonicalizeHostInfo(
        const char *input, int len, int default_port, Component *host, Component *port, std::string *output)
{
...
void canonicalizePath(const char *input, int len, Component *path, std::string *output)
{
    ...
    for (int i = path->begin; i < path->end(); i++) {
        ...
        if (ch == '.') {
            ...
            if (output->empty() || output->at(output->length() - 1) != '/') *output += ch;
        } else {
